...
export const ThemeToggle = ({ defaultTheme, className = '' }: ThemeToggleProps) => {
  ...
  if (isLoading) {
    return (
      <button
        className={twMerge(
          "p-3 rounded-full transition-all duration-200 cursor-not-allowed opacity-50 backdrop-blur-sm border border-white/20 shadow-lg",
          className
        )}
  ...
  return (
    <button
      onClick={toggleTheme}
      className={twMerge(
        "p-3 cursor-pointer rounded-full transition-all duration-200 bg-zinc-900 hover:bg-zinc-900/50 backdrop-blur-sm border border-white/20 shadow-lg hover:shadow-xl hover:scale-105",
        className
      )}
